...
module.exports = {
  async up(queryInterface, Sequelize) {
    await queryInterface.sequelize.query(`
      DROP VIEW IF EXISTS view_kardex_detalle CASCADE;
      CREATE VIEW view_kardex_detalle AS
        SELECT
            ROW_NUMBER() OVER (ORDER BY movimientos.date, movimientos.type) AS id,
            movimientos.type,
            movimientos.date,
          ...
          COALESCE(SUM(CASE WHEN movimientos.type = 'INPUT' THEN movimientos.quantity ELSE -movimientos.quantity END)
            OVER (
                ...
                ORDER BY movimientos.date, movimientos.id
                ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING
            ), 0) AS saldo_inicial,

            -- Saldos acumulados
            SUM(CASE WHEN movimientos.type = 'INPUT' THEN movimientos.quantity ELSE -movimientos.quantity END)
                OVER (PARTITION BY movimientos.id_product, movimientos.id_sucursal, movimientos.id_storage ORDER BY movimientos.date, movimientos.id) AS saldo,
            ...
            END)
                OVER (PARTITION BY movimientos.id_product, movimientos.id_sucursal, movimientos.id_storage ORDER BY movimientos.date, movimientos.id) AS cost_saldo
        ...
        FROM (
            ...
            SELECT
                ...
                i.id_sucursal,
                i.id_storage,
                di.id AS id  -- importante para ordenar los movimientos
            ...
            SELECT
                dt.id_product,
                LEAST(COALESCE(dt.quantity_received, dt.quantity), dt.quantity) AS quantity,
                'INPUT' AS type,
